...
export async function list(req: Request, res: Response) {
    ...
    sendSuccess(res, eventTypes, 200, "Event types fetched successfully")
}

export async function getById(req: Request, res: Response) {
    const { id } = req.params
    const eventType = await getEventTypeByIdService(Number(id), req.userId!)
    sendSuccess(res, eventType, 200, "Event type fetched successfully")
}
...
export async function getPublicEventType(req: Request, res: Response) {
    ...
    sendSuccess(res, eventType, 200, "Public event type fetched successfully")
}
